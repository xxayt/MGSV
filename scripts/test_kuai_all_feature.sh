...
# NOTE: modify it every time!!!
NAME="[your dir name in training phase]"
CUDA_VISIBLE_DEVICES=${GPU_DEVICE} python -m torch.distributed.launch --nproc_per_node=1 --master_port 1110${GPU_DEVICE} \
    test-MaDe.py --name ${NAME} \
    --do_eval \
    ...
    --batch_size_val ${VAL_BS} --num_workers 32 --epochs ${EPOCHS} \
    --distance_type COS \
    --test_csv ./dataset/MGSV-EC/test_data.csv \
    --frozen_feature_path ./features/Kuai_feature \
    --output_dir ./logs \
    --load_uni_model_path ./logs/kuai50k_uni/${NAME} \
    --num_display 10 --tb_writer 1 --save_model 0 --save_json 0 --test_best 1