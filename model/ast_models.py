...
class ASTModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(2, 3)  # Transpose to match model's expected input shape

        B = x.shape[0]  # Batch size
        x = self.v.patch_embed(x)  # Apply patch embedding
        # Expand cls_tokens and dist_token for the batch and concatenate with the patch embeddings
        ...
        all_features = x
        cls_dist_token = (x[:, 0] + x[:, 1]) / 2
        x = self.mlp_head(cls_dist_token)
        return x, cls_dist_token, all_features
